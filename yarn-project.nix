...
{ lib, nodejs, stdenv, fetchurl, writeText, fd }:
...
let
  ...
  cacheDrvs = let
    builder = builtins.toFile "builder.sh" ''
      ...
      yarn_cach_folder=.yarn/cache
      mkdir -p $yarn_cach_folder

      HOME="$TMP" \
        YARN_CACHE_FOLDER="$yarn_cach_folder" \
        CI=1 \
        node ${yarnPath} nixify fetch-one $locator

      # Because we change the cache dir, Yarn may generate a different name.
      mv "$yarn_cach_folder/$(sed 's/-[^-]*\.[^-]*$//' <<< "$outputFilename")"-* $out
    '';
  ...
  mkIsolatedBuild = { pname, version, reference, locators }: stdenv.mkDerivation (drvCommon // {
    ...
    buildPhase = ''
      echo running isolated build for: ${pname}: ${reference}
      ...
      # Using reference instead of version since version would not work for
      # github deps.
  ...
  project = stdenv.mkDerivation (drvCommon // {
    ...
    buildInputs =  drvCommon.buildInputs ++ [ fd ];
    ...
    configurePhase = ''
      ...
      yarn nixify inject-build \
        "node-module-test@https://github.com/adrian-gierakowski/node-module-test.git#commit=6c328a6f8716aa29368421a826f168f87b404ed5" \
        ${isolated."node-module-test@https://github.com/adrian-gierakowski/node-module-test.git#commit=6c328a6f8716aa29368421a826f168f87b404ed5"} \
        ".yarn/unplugged/node-module-test-https-497c75bc46/node_modules/node-module-test"
      ...
      echo; echo ls -al '${cacheFolder}'
      ls -al '${cacheFolder}'
      echo; echo fd . --max-depth 3 .yarn/unplugged
      fd . --max-depth 3 .yarn/unplugged
...
cacheEntries = {
"node-module-test@https://github.com/adrian-gierakowski/node-module-test.git#commit=6c328a6f8716aa29368421a826f168f87b404ed5" = { filename = "node-module-test-https-497c75bc46-9fcb4f37a6.zip"; sha512 = "9fcb4f37a6502fd2fc5e474c69dd15cc4fcac39978040cae4599b1bfd3774d13880d8c651ad4ba2deebb2fad558b204f23d4e3eb24a9eee7ea6de34ce81448c3"; };
